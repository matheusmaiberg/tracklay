...
# Must be your worker domain without trailing slash
WORKER_BASE_URL=https://cdn.yourstore.com

# Script size limit for ReDoS protection
...
#### Option B: Using GTM with Meta Pixel

1. Go to your Meta Pixel tag in GTM
2. Update the **Script URL**:
   - **OLD:** `https://connect.facebook.net/en_US/fbevents.js`
   - **NEW:** `{{PROXY_DOMAIN}}/cdn/f/{FACEBOOK_UUID}.js`
   - Replace `{FACEBOOK_UUID}` with your actual UUID from `/endpoints`

3. Update the **Event Endpoint**:
   - **OLD:** `https://www.facebook.com/tr`
   - **NEW:** `{{PROXY_DOMAIN}}/cdn/f/{FACEBOOK_UUID}` (POST endpoint)
...
2. Verify GTM loads from your proxy:
   - Open Network tab in browser dev tools
   - Look for request to: `{{PROXY_DOMAIN}}/cdn/g/{GOOGLE_UUID}?id=GTM-XXXX`
   - Should return 200 with JavaScript content
   - Should NOT be `googletagmanager.com/gtm.js`

3. Verify tracking events go to your proxy:
   - Look for POST to: `{{PROXY_DOMAIN}}/cdn/g/{GOOGLE_UUID}`
   - Should contain `v=2&tid=G-XXXX&...` parameters
...
**400 Bad Request with security-related message**

**Common Causes & Solutions:**

1. **Invalid tagId Format**
   - Container IDs must follow strict patterns:
     - GTM: `GTM-XXXXXX` (letters, numbers, hyphens)
     - GA4: `G-XXXXXXXXXX` (alphanumeric)
     - Meta Pixel: Numeric only
   - Check your container ID format in GTM/GA settings

2. **Invalid workerOrigin**
   - Requests must come from configured `ALLOWED_ORIGINS`
   - Check wrangler.toml includes all your domains:
     - `.myshopify.com` domain
     - Custom domain (if using one)
     - CDN subdomain (if applicable)

3. **Script Too Large (10MB limit)**
   - Security feature to prevent ReDoS attacks
   - Standard tracking scripts are <500KB
   - If legitimate script is blocked, contact support

4. **Invalid UUID Format**
   - UUIDs must be 12-64 hexadecimal characters
   - Check UUIDs returned from `/endpoints` endpoint
