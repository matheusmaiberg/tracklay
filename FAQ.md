...
|----------|--------|-------|
| **Google Analytics 4 (GA4)** | ✅ Supported | Via GTM or direct gtag.js |
| **Google Tag Manager** | ✅ Supported | Web container + Server-Side optional |
| **Meta Pixel (Facebook)** | ✅ **Supported** | Full support via Full Script Proxy |
| **TikTok Pixel** | ⚠️ Planned | Coming in future release |
| **Pinterest Tag** | ⚠️ Planned | Coming in future release |
| **Snapchat Pixel** | ⚠️ Planned | Coming in future release |

**Note**: Meta Pixel support includes:
- fbevents.js proxy
- /tr event endpoint proxy
...
Custom paths are no longer configurable via code. The worker uses dynamic UUID-based routing (`/x/{uuid}`) for security and anti-blocking purposes.

If you need custom paths for specific use cases:

1. **For static assets**: Use `/lib/` endpoint
   ```
   /lib/script-name → proxied to configured library URL
   ```

2. **For dynamic endpoints**: Use the `/endpoints` API to get current UUIDs
   ```bash
   curl -H "Authorization: Bearer $TOKEN" https://yourstore.com/endpoints
   ```

3. **For advanced routing**: Modify `src/routing/router.js` directly
   ```javascript
   // Add custom route handling
   if (pathname.startsWith('/custom/')) {
     return handleCustomProxy(request);
   }
   ```
...
**Symptoms**: 429 Too Many Requests errors in console or logs

**Understanding Stratified Rate Limiting**

Tracklay uses **IP + endpoint-based rate limiting**:
- Each endpoint has its own independent counter
- Example: `/g/collect` and `/tr/{uuid}` are counted separately
- One endpoint being hit won't affect others

**Solutions:**

1. **Increase limits in wrangler.toml**:
   ```toml
   RATE_LIMIT_REQUESTS = "200"
   RATE_LIMIT_WINDOW = "60000"
   ```

2. **Check which endpoint is being rate limited**:
   - Look at the URL in 429 error response
   - Check `X-RateLimit-Endpoint` header in response

3. **Verify it's not bot traffic**:
   - Check Cloudflare Analytics for traffic patterns
   - Look for unusual User-Agent strings
   - Verify request rates per IP

4. **For high-traffic stores**:
   - Consider increasing to 500+ req/min
   - Monitor actual usage patterns
   - Use Cloudflare Analytics to set appropriate limits
...
- **Quick Start**: [docs/organizar/QUICK_START.md](../../docs/organizar/QUICK_START.md)
